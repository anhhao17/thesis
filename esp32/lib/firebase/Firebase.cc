...
bool FireBase::connect(const char *state){
    ...
    if(m_wifi_client->connect("smarthomeapp-d2e21-default-rtdb.firebaseio.com",443)){
        Serial.printf("INFO: file: %s:%d, msg: Send POST request\n",__FILE__,__LINE__);
        m_wifi_client->printf("%s /Users/iSRxswqPiwcVicxtKfiqvDMEhKJ2/device.json HTTP/1.1\n",state);
...
void FireBase::sendData(String path,bool val){
    ...
    if(connect("PATCH")){
        ...
        if(json.readFrom(*m_wifi_client)){
            ...
            Serial.printf("INFO: file: %s:%d, msg: Completed\n",__FILE__,__LINE__);
        }else
            Serial.printf("ERROR: file: %s:%d, msg: Failed\n",__FILE__,__LINE__);
        m_wifi_client->stop();
    }
    json.clear();
