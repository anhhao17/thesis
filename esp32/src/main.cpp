 ...
 * 4. APP FLOW {
 * Task Application waiting for Task Mic store full buffer 
 * Task App perform 1s Buffer to detect wake word -> if yes -> change state to recognize -> store 3s audio send to Wit.ai server -> get Json file
 * Analyze Json file -> On/Off/Update/do some command 
 * 
 * }
 **/
void setup()
{
	...
	while ((WiFi.status() != WL_CONNECTED))
  	{
		indicator_light->setState(indicator::PULSING);
    	Serial.print(".");
    	delay(100);
    	handleClient();
  	}  
	indicator_light->setState(indicator::OK);
